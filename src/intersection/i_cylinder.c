...
/*   Updated: 2022/10/13 16:10:11 by jbedaux          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	get_cylinder_normal(t_ray ray, t_object *cylinder, t_t4 t_)
{
	...
	orientation = normalize_vector(cylinder->orientation);
...
static t_t4	create_infinite_cylinder(t_ray ray, t_object *cylinder)
{
	...
	cross1 = cross(ray.direction, cylinder->orientation);
	
	// deze subtract geeft een vector van cylinder.center naar ray.origin
	// dus ray.direction maar dan de andere kant op
	t_xyz center_to_origin = substract_vectors(ray.origin, cylinder->center);

	cross2 = cross(center_to_origin, cylinder->orientation);
	a = get_dot_product(cross1, cross1);
	b = 2 * get_dot_product(cross1, cross2);
	c = get_dot_product(cross2, cross2) - (pow(cylinder->radius, 2) 
		* get_dot_product(cylinder->orientation, cylinder->orientation));
	det = pow(b, 2) - (4 * a * c);
	if (det < 0)
		return (t);
	a = 2 * a;
